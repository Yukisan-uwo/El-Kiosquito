...
- **RF-CVI-027** *(añadido en enmienda v1.6, control de stock e imágenes en POS)*: El punto de venta DEBE desplegar el stock actual en tiempo real de cada producto para la sucursal activa, distinguiendo visualmente productos agotados (`stock = 0`), con stock bajo (`stock <= 5`) y stock disponible.
- **RF-CVI-028** *(añadido en enmienda v1.6)*: El catálogo del POS DEBE mostrar una representación visual / imagen ilustrativa por producto para facilitar la rápida identificación física por parte del cajero durante la atención.
...
- **RN-CVI-012** *(añadida en enmienda v1.6)*: En el punto de venta, el sistema NO DEBE permitir agregar al carrito productos con stock cero (`stock_actual = 0`) ni permitir que la cantidad total agregada o modificada en el carrito supere el stock físico disponible en la sucursal. Los controles de incremento (steppers) deben deshabilitarse al alcanzar el tope de existencias.
