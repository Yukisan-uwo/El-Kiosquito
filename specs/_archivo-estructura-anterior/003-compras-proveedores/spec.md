...
Este módulo gestiona el ciclo completo de una orden de compra a un proveedor: registro del proveedor, creación de la orden, recepción de mercadería (que puede llegar en más de un envío) y el historial de costos que alimenta el cálculo de margen real (OT1.1, propiedad de Precios y Márgenes). Incluye además el control de negocio más particular del enunciado para este departamento: una compra "por oferta" del proveedor no puede confirmarse a ciegas sin haber consultado el pronóstico de demanda del producto, para evitar comprar barato algo que luego se convierte en stock muerto.
...
4. **Dado** una recepción registrada, **cuando** el sistema la procesa, **entonces** también crea un registro en `historial_costo_producto` con el costo pagado, el proveedor y la fecha, para alimentar el cálculo de margen real de Precios y Márgenes (OT1.1).
...
6. **Dado** una orden marcada `es_oferta = true`, **cuando** intento pasarla a `recibida_parcial` sin haber consultado el pronóstico de demanda para cada producto de la orden, **entonces** el sistema rechaza el cambio de estado (RN-CP-001).
...
- **¿Qué pasa si** un proveedor se desactiva mientras tiene órdenes de compra pendientes? → La desactivación (OO-CP09) solo bloquea la creación de *nuevas* órdenes con ese proveedor; las órdenes ya creadas se pueden seguir recibiendo con normalidad.
- **¿Qué pasa si** dos proveedores distintos cotizan el mismo producto el mismo día a precios diferentes? → Ambos quedan registrados en `historial_costo_producto` sin conflicto — es exactamente el dato que necesita OO-CP06 para comparar precios entre proveedores.
...
- **RF-CP-009**: El sistema DEBE permitir consultar el pronóstico de demanda de un producto antes de confirmar una orden marcada como compra por oferta.
...
- **RN-CP-001**: Una orden con `es_oferta = true` no puede cambiar de `pendiente` a `recibida_parcial` o `recibida_completa` si algún `detalle_orden_compra` no tiene `pronostico_consultado = true`; si la cantidad pedida excede la cantidad recomendada por el pronóstico, `motivo_no_siguio_pronostico` es obligatorio para ese detalle.

## Entidades Clave

- **Proveedor**: nombre, contacto, estado activo/inactivo.
- **OrdenCompra**: proveedor, sucursal destino, fecha, estado (derivado), `es_oferta`.
- **DetalleOrdenCompra**: producto, cantidad pedida, precio ofrecido, cantidad recibida (derivada), datos de validación de pronóstico.
- **RecepcionOrdenCompra**: evento individual de recepción — permite entregas parciales en más de un envío.
- **HistorialCostoProducto**: costo pagado por producto/proveedor/fecha — alimenta OT1.1 (margen real) y OO-CP05/06.
